...
function register($pseudo, $email, $pass, $confPass , $hash){
        ...
        if ($pseudo && $emailConf && $validPass) {
            ...
            $_SESSION['users'][] = $user;
            $_SESSION['error'][0] = 'utilisateur enregistré';
            header("Location: login.php");
            die;
        //////////////////////Tableau erreur/////////////////////
        } elseif (!$pseudo) {
            $_SESSION['error'][0] = '<p>Pseudo invalide</p>';
        } elseif (!$emailConf) {
            $_SESSION['error'][0] = '<p>E-mail invalide ou déjà utilisé</p>';
        } elseif (!$validPass) {
             $_SESSION['error'][0] = '<p>Mot de passe invalide</p>';
        } else {
...
function login($email, $pass){
    ...
    if ($searchEmail) {
        ...
        $_SESSION['error'][0] = '<p>E-mail invalide</p>';
        header('Location:login.php');
        die;
    }
    return;
